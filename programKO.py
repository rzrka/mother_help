...
TYPE_CLOTHES_EXCLUDED = 'перчатки|сигн'
                ...
                ('руб', 'рубаш', 'рубашка'):
                  {
                       ...
                       ('к/р', 'кор.рук', 'кор.т.с.', 'кор.с.р', 'кор.р', 'к/рук'):
                           ...
                           ('пар',):
                                'Рубашка мужская с коротким рукавом парадная'
                           },
                       ('руб', 'рубаш', 'рубашка'):
                           {('повседневаная', 'пов', 'гол'):
                                'Рубашка с коротким рукавом повседневная голубого цвета',
                  ...
                  ('Джемпер',): 'Джемпер трикотажный мужской',
                  ('куртка',):
                    {
                        ('вет',): 'Куртка ветровка',
                        ('зим',): 'Куртка зимняя'
...
def clothes(tabs):
    ...
    f = open('Одежды который нет в словаре.txt', 'w')
    df = pd.read_excel('data/бухКО.xlsx',
                       skiprows=2)
    tab_id = df['   Таб.№']
    cloths = df['Название основного средства']
    dates = df['Д/оприход.']
    for tab_el, cloth, date in zip(tab_id, cloths, dates):
